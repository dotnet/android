...
sealed class JavaPeerScanner : IDisposable
{
	...
	void ScanAssembly (AssemblyIndex index, Dictionary<string, JavaPeerInfo> results)
	{
		foreach (var typeHandle in index.Reader.TypeDefinitions) {
			...
			var peer = new JavaPeerInfo {
				...
				IsImplementorOrEventDispatcher = DetectImplementorOrEventDispatcher (jniName, fullName),
			};
	...
	/// <summary>
	/// Detects binding-generated Implementor and EventDispatcher types.
	/// These are generated by the Java.Interop binding generator and always have:
	///   - JNI name starting with "mono/" (binding generator convention)
	///   - Managed name ending with "Implementor" or "EventDispatcher"
	/// Requiring both conditions avoids misclassifying user types.
	/// </summary>
	static bool DetectImplementorOrEventDispatcher (string jniName, string managedTypeName)
	{
		return jniName.StartsWith ("mono/", StringComparison.Ordinal) &&
			(managedTypeName.EndsWith ("Implementor", StringComparison.Ordinal) ||
			 managedTypeName.EndsWith ("EventDispatcher", StringComparison.Ordinal));
	}
