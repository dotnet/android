...
bool Zip::zip_load_entry_common (size_t entry_index, std::vector<uint8_t> const& buf, dynamic_local_string<SENSIBLE_PATH_MAX> &entry_name, zip_scan_state &state) noexcept
{
	...
	log_debug (LOG_ASSEMBLY, "%.*s entry: %s", static_cast<int>(state.file_name.length ()), state.file_name.data (), optional_string (entry_name.get (), "unknown"));
...
void Zip::scan_archive (std::string_view const& apk_path, ScanCallbackFn entry_cb) noexcept
{
	...
	log_debug (LOG_ASSEMBLY, "APK %.*s FD: %d", static_cast<int>(apk_path.length ()), apk_path.data (), fd);
	...
	if (close (fd) < 0) {
		log_warn (
			LOG_ASSEMBLY,
			"Failed to close file descriptor for %.*s. %s",
			static_cast<int>(apk_path.length ()),
			apk_path.data (),
			strerror (errno)
		);
	}
