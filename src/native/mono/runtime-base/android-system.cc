...
AndroidSystem::monodroid_get_system_property (std::string_view const& name, char *value, size_t value_size) noexcept
{
	...
	value [0] = '\0';
	if (value_size < PROPERTY_VALUE_BUFFER_LEN) {
		return -1;
	}
...
AndroidSystem::monodroid_get_system_property_from_overrides ([[maybe_unused]] const char *name, [[maybe_unused]] char ** value) noexcept
{
	...
	for (const char *od : override_dirs) {
		...
		log_info (LOG_DEFAULT, "Trying to get property from {}", override_file.get ());
		...
		log_info (LOG_DEFAULT, "Property '{}' from {} has value '{}'.", name, od, *value);
		return result;
	}
#endif // def DEBUG
	return 0;
...
AndroidSystem::get_max_gref_count_from_system (void) noexcept
{
	...
	if (monodroid_get_system_property (SharedConstants::DEBUG_MONO_MAX_GREFC, override) > 0) {
		...
		if (*e) {
			log_warn (LOG_GC, "Unsupported '{}' value '{}'.", SharedConstants::DEBUG_MONO_MAX_GREFC.data (), override.get ());
		}
...
AndroidSystem::setup_environment () noexcept
{
	if (is_mono_aot_enabled () && *mono_aot_mode_name != '\0') {
		...
		if (aotMode != MonoAotMode::MONO_AOT_MODE_LAST) {
			log_debug (LOG_DEFAULT, "Mono AOT mode: {}", mono_aot_mode_name);
		} else {
			if (!is_interpreter_enabled ()) {
				log_warn (LOG_DEFAULT, "Unknown Mono AOT mode: {}", mono_aot_mode_name);
			} else {
	...
	for (const char *od : override_dirs) {
		...
		if (Util::file_exists (env_override_file.get ())) {
			log_debug (LOG_DEFAULT, "Loading {}", env_override_file.get ());
...
AndroidSystem::detect_embedded_dso_mode (jstring_array_wrapper& appDirs) noexcept
{
	...
	if (!Util::file_exists (libmonodroid_path.get ())) {
		...
		log_debug (LOG_ASSEMBLY, "Native libs extracted to {}, assuming application/android:extractNativeLibs == true", appDirs[SharedConstants::APP_DIRS_DATA_DIR_INDEX].get_cstr ());
...
AndroidSystem::setup_app_library_directories (jstring_array_wrapper& runtimeApks, jstring_array_wrapper& appDirs, bool have_split_apks) noexcept
{
	if (!is_embedded_dso_mode_enabled ()) {
		...
		log_debug (LOG_ASSEMBLY, "Added filesystem DSO lookup location: {}", appDirs[SharedConstants::APP_DIRS_DATA_DIR_INDEX].get_cstr ());
	} else {
...
AndroidSystem::add_apk_libdir (const char *apk, size_t &index, const char *abi) noexcept
{
	...
	log_debug (LOG_ASSEMBLY, "Added APK DSO lookup location: {}", app_lib_directories[index]);
	index++;
}
