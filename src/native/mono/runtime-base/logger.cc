...
namespace {
	...
	open_file (LogCategories category, const char *path, const char *override_dir, const char *filename)
	{
		...
		if (path && access (path, W_OK) < 0) {
			log_warn (category,
				"Could not open path '%s' for logging (\"%s\"). Using '%s/%s' instead.",
		...
		if (f) {
			...
			log_warn (category, "Could not open path '%s' for logging: %s", optional_string (path), strerror (errno));
		}
...
Logger::set_debugger_log_level (const char *level) noexcept
{
	...
	if (v > std::numeric_limits<int>::max ()) {
		log_warn (LOG_DEFAULT, "Debugger log level value is higher than the maximum of %d, resetting to the maximum value.", std::numeric_limits<int>::max ());
