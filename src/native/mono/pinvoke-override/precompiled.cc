...
PinvokeOverride::monodroid_pinvoke_override (const char *library_name, const char *entrypoint_name)
{
	...
	if (library_name_hash == java_interop_library_hash || library_name_hash == xa_internal_api_library_hash) {
		...
		if (entry == nullptr) [[unlikely]] {
			log_fatal (LOG_ASSEMBLY, "Internal p/invoke symbol '{} @ {}' (hash: {:x}) not found in compile-time map.", library_name, entrypoint_name, entrypoint_hash);
			...
			for (size_t i = 0uz; i < internal_pinvokes_count; i++) {
				...
				log_fatal (LOG_ASSEMBLY, "\t'{}'={:p} (hash: {:x})", e.name, e.func, e.hash);
			}
	...
	if (dotnet_dso_handle != nullptr) {
		...
		if (entry != nullptr) {
			...
			if (entry->func == nullptr) {
				log_fatal (LOG_ASSEMBLY, "Failed to load symbol '{}' from shared library '{}'", entrypoint_name, library_name);
		...
		log_debug (LOG_ASSEMBLY, "Symbol '{}' in library '{}' not found in the generated tables, falling back to slow path", entrypoint_name, library_name);
	}
