...
MonodroidRuntime::open_from_update_dir (MonoAssemblyName *aname, [[maybe_unused]] char **assemblies_path, [[maybe_unused]] void *user_data)
{
	...
	for (const char *override_dir : AndroidSystem::override_dirs) {
		...
		log_debug (LOG_ASSEMBLY, "open_from_update_dir: trying to open assembly: %s", optional_string (fullpath.get ()));
		if (Util::file_exists (fullpath.get ())) {
			...
			if (result == nullptr || status != MonoImageOpenStatus::MONO_IMAGE_OK) {
				log_warn (LOG_ASSEMBLY, "Failed to load managed assembly '%s'. %s", optional_string (fullpath.get ()), mono_image_strerror (status));
			}
		} else {
	...
	while (runtime_args.next_token (',', token)) {
		if (token.starts_with (ARG_DEBUG)) {
			...
			if (sdb_port < 0 || sdb_port > std::numeric_limits<unsigned short>::max ()) {
				log_error (LOG_DEFAULT, "Invalid SDB port value %u", sdb_port);
				ret = false;
				continue;
			}

			if (out_port > std::numeric_limits<unsigned short>::max ()) {
				log_error (LOG_DEFAULT, "Invalid output port value %u", out_port);
				ret = false;
				continue;
			}
			...
			log_error (LOG_DEFAULT, "Unknown runtime argument: '%s'", optional_string (arg.get ()));
			ret = false;
		}
	}
...
MonodroidRuntime::mono_runtime_init ([[maybe_unused]] JNIEnv *env, [[maybe_unused]] dynamic_local_string<PROPERTY_VALUE_BUFFER_LEN>& runtime_args) noexcept
{
	...
	if (!parse_runtime_args (runtime_args, &options)) {
		log_error (LOG_DEFAULT, "Failed to parse runtime args: '%s'", optional_string (runtime_args.get ()));
	} else if (options.debug && cur_time > options.timeout_time) {
		log_warn (LOG_DEBUGGER, "Not starting the debugger as the timeout value has been reached; current-time: %lld; timeout: %lld", static_cast<long long>(cur_time), static_cast<long long>(options.timeout_time));
	} else if (options.debug && cur_time <= options.timeout_time) {
		EmbeddedAssemblies::set_register_debug_symbols (true);
		...
		log_warn (LOG_DEBUGGER, "Trying to initialize the debugger with options: %s", optional_string (debug_arg));
	...
	if (AndroidSystem::monodroid_get_system_property (SharedConstants::DEBUG_MONO_RUNTIME_ARGS_PROPERTY, prop_val) > 0) {
		...
		log_warn (LOG_DEBUGGER, "passing '%s' as extra arguments to the runtime.", optional_string (prop_val.get ()));
...
MonodroidRuntime::create_domain (JNIEnv *env, jstring_array_wrapper &runtimeApks, bool is_root_domain, bool have_split_apks) noexcept
{
	...
	if (user_assemblies_count == 0 && AndroidSystem::count_override_assemblies () == 0) {
		...
		log_fatal (LOG_DEFAULT,
			"No assemblies found in '%s' or '%s'. Assuming this is part of Fast Deployment. Exiting...",
...
MonodroidRuntime::init_android_runtime (JNIEnv *env, jclass runtimeClass, jobject loader) noexcept
{
	...
	if (initialize == nullptr) {
		log_fatal (LOG_DEFAULT, "Failed to get pointer to Initialize. Mono error: %s", optional_string (mono_error_get_message (&error)));
	}
...
MonodroidRuntime::set_environment_variable_for_directory (const char *name, jstring_wrapper &value, bool createDirectory, mode_t mode) noexcept
{
	if (createDirectory) {
		...
		if (rv < 0 && errno != EEXIST)
			log_warn (LOG_DEFAULT, "Failed to create directory for environment variable %s. %s", optional_string (name), strerror (errno));
	}
...
MonodroidRuntime::create_xdg_directory (jstring_wrapper& home, size_t home_len, std::string_view const& relative_path, std::string_view const& environment_variable_name) noexcept
{
	...
	log_debug (LOG_DEFAULT, "Creating XDG directory: %s", optional_string (dir.get ()));
	int rv = Util::create_directory (dir.get (), DEFAULT_DIRECTORY_MODE);
	if (rv < 0 && errno != EEXIST)
		log_warn (LOG_DEFAULT, "Failed to create XDG directory %s. %s", optional_string (dir.get ()), strerror (errno));
...
MonodroidRuntime::set_debug_env_vars (void) noexcept
{
	...
	auto log_envvar = [](const char *name, const char *v) {
		log_debug (LOG_DEFAULT, "Env variable '%s' set to '%s'.", optional_string (name), optional_string (v));
	};
	...
	while (value.next_token ('|', arg_token)) {
		...
		if (idx < 0 || index == arg.length () - 1) {
			...
			// ’=value’
			log_warn (LOG_DEFAULT, "Attempt to set environment variable without specifying name: '%s'", optional_string (arg.get ()));
		} else {
			// ’name=value’
			arg[index] = '\0';
...
MonodroidRuntime::set_profile_options () noexcept
{
	...
	if (Util::create_directory (AndroidSystem::override_dirs[0], 0) < 0) {
		log_warn (LOG_DEFAULT, "Failed to create directory '%s'. %s", optional_string (AndroidSystem::override_dirs[0]), std::strerror (errno));
	}

	log_warn (LOG_DEFAULT, "Initializing profiler with options: %s", optional_string (value.get ()));
                                                          ...
                                                          jboolean haveSplitApks) noexcept
{
	...
	if (runtimeNativeLibDir != nullptr) {
		...
		log_debug (LOG_DEFAULT, "Using runtime path: %s", optional_string (AndroidSystem::get_runtime_libdir ()));
	}
	...
	if (Util::should_log (LOG_DEFAULT)) [[unlikely]] {
		log_info_nocheck_fmt (
			...
			static_cast<int>(BuildInfo::xa_version.length ()), BuildInfo::xa_version.data (),
			static_cast<int>(BuildInfo::architecture.length ()), BuildInfo::architecture.data (),
			static_cast<int>(BuildInfo::kind.length ()), BuildInfo::kind.data (),
			static_cast<int>(BuildInfo::date.length ()), BuildInfo::date.data (),
			static_cast<int>(BuildInfo::ndk_version.length ()), BuildInfo::ndk_version.data (),
			static_cast<int>(BuildInfo::ndk_api_level.length ()), BuildInfo::ndk_api_level.data (),
