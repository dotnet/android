...
uint64_t
MonodroidRuntime::get_java_class_name_hash_for_TypeManager (jclass klass) noexcept
{
	if (klass == nullptr || Class_getName == nullptr)
		return 0;
	...
	// Notes:
	// - the JNI name uses slashes (/) as separators, not dots (.)
	// - the JNI name is (modified) UTF-8 encoded

	uint64_t ret = static_cast<uint64_t> (xxhash::hash (mutf8, strlen (mutf8)));
