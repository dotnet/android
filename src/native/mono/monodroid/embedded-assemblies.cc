...
EmbeddedAssemblies::map_runtime_file (XamarinAndroidBundledAssembly& file) noexcept
{
	...
	if (MonodroidState::is_startup_in_progress ()) {
		...
		if (already_mapped) {
			log_debug (LOG_ASSEMBLY, "Assembly {} already mmapped by another thread, unmapping our copy", file.name);
	...
	bool ref_only) noexcept
{
	...
	if (strcmp (assembly.name, name.get ()) != 0) {
		if (strcmp (assembly.name, abi_name.get ()) != 0) {
			...
			log_debug (LOG_ASSEMBLY, "open_from_bundles: found architecture-specific: '{}'", abi_name.get ());
		}
	}
...
EmbeddedAssemblies::individual_assemblies_open_from_bundles (dynamic_local_string<SENSIBLE_PATH_MAX>& name, TLoaderData loader_data, bool ref_only) noexcept
{
	...
	log_debug (LOG_ASSEMBLY, "individual_assemblies_open_from_bundles: looking for bundled name: '{}'", name.get ());
...
EmbeddedAssemblies::assembly_store_open_from_bundles (dynamic_local_string<SENSIBLE_PATH_MAX>& name, TLoaderData loader_data, bool ref_only) noexcept
{
	...
	log_debug (LOG_ASSEMBLY, "assembly_store_open_from_bundles: looking for bundled name: '{}' (hash {:x})", name.get (), name_hash);
...
EmbeddedAssemblies::typemap_java_to_managed ([[maybe_unused]] hash_t hash, const MonoString *java_type) noexcept
{
	...
	if (type == nullptr) [[unlikely]] {
		log_info (LOG_ASSEMBLY, "typemap: managed type '{}' (mapped from Java type '{}') could not be loaded", managed_type_name, java_type_name.get ());
		return nullptr;
	}

	MonoReflectionType *ret = mono_type_get_object (Util::get_current_domain (), type);
	if (ret == nullptr) [[unlikely]] {
		log_warn (LOG_ASSEMBLY, "typemap: unable to instantiate managed type '{}'", managed_type_name);
		return nullptr;
	}
...
EmbeddedAssemblies::typemap_java_to_managed (hash_t hash, const MonoString *java_type_name) noexcept
{
	...
	if (module->image == nullptr) {
		...
		if (module->image == nullptr) {
			log_debug (LOG_ASSEMBLY, "typemap: assembly '{}' hasn't been loaded yet, attempting a full load", module->assembly_name);
		...
		if (module->image == nullptr) {
			log_error (LOG_ASSEMBLY, "typemap: unable to load assembly '{}' when looking up managed type corresponding to Java type '{}'", module->assembly_name, to_utf8 (java_type_name).get ());
			return nullptr;
		}
	}
	...
	if (klass == nullptr) [[unlikely]] {
		log_error (LOG_ASSEMBLY, "typemap: unable to find managed type with token ID {} in assembly '{}', corresponding to Java type '{}'", java_entry->type_token_id, module->assembly_name, to_utf8 (java_type_name).get ());
		return nullptr;
	}
	...
	if (ret == nullptr) {
		log_warn (LOG_ASSEMBLY, "typemap: unable to instantiate managed type with token ID {} in assembly '{}', corresponding to Java type '{}'", java_entry->type_token_id, module->assembly_name, to_utf8 (java_type_name).get ());
		return nullptr;
	}
...
EmbeddedAssemblies::typemap_managed_to_java ([[maybe_unused]] MonoType *type, MonoClass *klass, [[maybe_unused]] const uint8_t *mvid) noexcept
{
	...
	if (entry == nullptr) [[unlikely]] {
		log_info (LOG_ASSEMBLY, "typemap: unable to find mapping to a Java type from managed type '{}'", full_name.get ());
		return nullptr;
	}
...
EmbeddedAssemblies::typemap_managed_to_java ([[maybe_unused]] MonoType *type, MonoClass *klass, const uint8_t *mvid) noexcept
{
	...
	if (entry->java_map_index >= java_type_count) [[unlikely]] {
		log_warn (LOG_ASSEMBLY, "typemap: type with token {} ({:x}) in module [{}] ({}) has invalid Java type index {}", token, token, MonoGuidString (mvid).get (), match->assembly_name, entry->java_map_index);
		return nullptr;
	}

	TypeMapJava const& java_entry = map_java[entry->java_map_index];
	if (java_entry.java_name_index >= java_type_count) [[unlikely]] {
		log_warn (LOG_ASSEMBLY, "typemap: type with token {} ({:x}) in module [{}] ({}) points to invalid Java type at index {} (invalid type name index {})", token, token, MonoGuidString (mvid).get (), match->assembly_name, entry->java_map_index, java_entry.java_name_index);
		return nullptr;
	}
...
EmbeddedAssemblies::gather_bundled_assemblies_from_apk (const char* apk, monodroid_should_register should_register) noexcept
{
	...
	log_debug (LOG_ASSEMBLY, "APK {} FD: {}", apk, fd);
...
EmbeddedAssemblies::typemap_read_header ([[maybe_unused]] int dir_fd, const char *file_type, const char *dir_path, const char *file_path, uint32_t expected_magic, H &header, size_t &file_size, int &fd)
{
	...
	if (res < 0) {
		log_error (LOG_ASSEMBLY, "typemap: failed to stat {} file '{}/{}': {}", file_type, dir_path, file_path, strerror (errno));
		return false;
	}

	file_size = static_cast<size_t>(sbuf.st_size);
	if (file_size < sizeof (header)) {
		log_error (LOG_ASSEMBLY, "typemap: {} file '{}/{}' is too small (must be at least {} bytes)", file_type, dir_path, file_path, sizeof (header));
		return false;
	}

	fd = openat (dir_fd, file_path, O_RDONLY);
	if (fd < 0) {
		log_error (LOG_ASSEMBLY, "typemap: failed to open {} file {}/{} for reading: {}", file_type, dir_path, file_path, strerror (errno));
		return false;
	}
	...
	if (nread <= 0) {
		if (nread < 0) {
			log_error (LOG_ASSEMBLY, "typemap: failed to read {} file header from '{}/{}': {}", file_type, dir_path, file_path, strerror (errno));
		} else {
			log_error (LOG_ASSEMBLY, "typemap: end of file while reading {} file header from '{}/{}'", file_type, dir_path, file_path);
		}
	...
	if (header.magic != expected_magic) {
		log_error (LOG_ASSEMBLY, "typemap: invalid magic value in the {} file header from '{}/{}': expected {:x}, got {:x}", file_type, dir_path, file_path, expected_magic, header.magic);
		return false;
	}

	if (header.version != MODULE_FORMAT_VERSION) {
		log_error (LOG_ASSEMBLY, "typemap: incompatible {} format version. This build supports only version {}, file '{}/{}' uses version {}", file_type, MODULE_FORMAT_VERSION, dir_path, file_path, header.version);
		return false;
	}
...
{
	log_debug (LOG_ASSEMBLY, "typemap: loading TypeMap index file '{}/{}'", dir_path, index_path);
...
EmbeddedAssemblies::typemap_load_file (BinaryTypeMapHeader &header, const char *dir_path, const char *file_path, int file_fd, TypeMap &module)
{
	...
	if (nread != static_cast<ssize_t>(header.assembly_name_length)) {
		log_error (LOG_ASSEMBLY, "typemap: failed to read map assembly name from '{}/{}': {}", dir_path, file_path, strerror (errno));
		return false;
	}
	...
	if (nread != static_cast<ssize_t>(data_size)) {
		log_error (LOG_ASSEMBLY, "typemap: failed to read map data from '{}/{}': {}", dir_path, file_path, strerror (errno));
		return false;
	}
...
{
	log_debug (LOG_ASSEMBLY, "typemap: loading TypeMap file '{}/{}'", dir_path, file_path);
...
EmbeddedAssemblies::register_from_apk (const char *apk_file, monodroid_should_register should_register) noexcept
{
	...
	log_info (LOG_ASSEMBLY, "Package '{}' contains {} assemblies", apk_file, number_of_found_assemblies - prev);
...
{
	log_debug (LOG_ASSEMBLY, "Looking for assemblies in '{}'", lib_dir_path);
	DIR *lib_dir = opendir (lib_dir_path); // TODO: put it in a scope guard at some point
	if (lib_dir == nullptr) {
		log_warn (LOG_ASSEMBLY, "Unable to open app library directory '{}': {}", lib_dir_path, std::strerror (errno));
		return 0;
	}
	...
	if (dir_fd < 0) [[unlikely]] {
		log_warn (LOG_ASSEMBLY, "Unable to obtain file descriptor for directory '{}': {}", lib_dir_path, std::strerror (errno));
		closedir (lib_dir);
		return 0;
	}
	...
	if (state.file_fd < 0) [[unlikely]] {
		log_warn (LOG_ASSEMBLY, "Unable to duplicate file descriptor {} for directory '{}': {}", dir_fd, lib_dir_path, std::strerror (errno));
		closedir (lib_dir);
		return 0;
	}
	...
	do {
		...
		if (cur == nullptr) {
			if (errno != 0) {
				log_warn (LOG_ASSEMBLY, "Failed to open a directory entry from '{}': {}", lib_dir_path, std::strerror (errno));
		...
		if (!runtime_config_blob_found && std::strncmp (cur->d_name, SharedConstants::RUNTIME_CONFIG_BLOB_NAME.data (), SharedConstants::RUNTIME_CONFIG_BLOB_NAME.size ()) == 0) {
			log_debug (LOG_ASSEMBLY, "Mapping runtime config blob from '{}'", cur->d_name);
