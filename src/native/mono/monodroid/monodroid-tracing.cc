...
MonodroidRuntime::log_traces (JNIEnv *env, TraceKind kind, const char *first_line) noexcept
{
	if (!tracing_init_done) {
		...
		if (handle == nullptr) {
			log_warn (LOG_DEFAULT, "Failed to load native tracing library '%.*s'. %s", static_cast<int>(SharedConstants::xamarin_native_tracing_name.length ()), SharedConstants::xamarin_native_tracing_name.data (), err == nullptr ? "Unknown error" : err);
		} else {
