                                     ...
                                     jstring runtimeNativeLibDir, jobjectArray appDirs, jboolean haveSplitApks)
{
	...
	if (runtimeNativeLibDir != nullptr) {
		...
		log_warn (LOG_DEFAULT, "Using runtime path: %s", optional_string (AndroidSystem::get_runtime_libdir ()));
	}
...
copy_file_to_internal_location (char *to_dir, char *from_dir, char *file)
{
	...
	do {
		...
		log_warn (LOG_DEFAULT,
			"Copying file `%s` from external location `%s` to internal location `%s`",
		...
		if (r < 0 && errno != ENOENT) {
			log_warn (LOG_DEFAULT, "Unable to delete file `%s`: %s", optional_string (to_file), strerror (errno));
			break;
		}

		if (!Util::file_copy (to_file, from_file)) {
			log_warn (LOG_DEFAULT, "Copy failed from `%s` to `%s`: %s", optional_string (from_file), optional_string (to_file), strerror (errno));
			break;
		}
...
copy_native_libraries_to_internal_location ()
{
	for (const char *od : AndroidSystem::override_dirs) {
		...
		log_warn (LOG_DEFAULT, "checking directory: `%s`", optional_string (dir_path));

		if (dir_path == nullptr || !Util::directory_exists (dir_path)) {
			log_warn (LOG_DEFAULT, "directory does not exist: `%s`", optional_string (dir_path));
			delete[] dir_path;
			continue;
		}

		if ((dir = ::opendir (dir_path)) == nullptr) {
			log_warn (LOG_DEFAULT, "could not open directory: `%s`", optional_string (dir_path));
			delete[] dir_path;
			continue;
		}

		while ((e = readdir (dir)) != nullptr) {
			log_warn (LOG_DEFAULT, "checking file: `%s`", optional_string (e->d_name));
...
runtime_exists (const char *dir, char*& libmonoso)
{
	...
	log_warn (LOG_DEFAULT, "Checking whether Mono runtime exists at: %s", optional_string (libmonoso));
	if (Util::file_exists (libmonoso)) {
		log_info (LOG_DEFAULT, "Mono runtime found at: %s", optional_string (libmonoso));
		return true;
	}
	delete[] libmonoso;
	libmonoso = nullptr;
...
get_libmonosgen_path ()
{
	...
	if (libmonoso != nullptr && Util::file_exists (libmonoso)) {
		...
		if (!Util::file_exists (link)) {
			...
			if (result != 0 && errno == EEXIST) {
				log_warn (LOG_DEFAULT, "symlink exists, recreating: %s -> %s", optional_string (link), optional_string (libmonoso));
				unlink (link);
				result = symlink (libmonoso, link);
			}
			if (result != 0)
				log_warn (LOG_DEFAULT, "symlink failed with errno=%d %s", errno, strerror (errno));
		}
		delete[] libmonoso;
		libmonoso = link;
	}

	log_warn (LOG_DEFAULT, "Trying to load sgen from: %s", optional_string (libmonoso));
	if (libmonoso != nullptr && Util::file_exists (libmonoso))
		return libmonoso;
	delete[] libmonoso;
	...
	if (runtime_exists (AndroidSystem::SYSTEM_LIB_PATH.data (), libmonoso))
		return libmonoso;
	log_fatal (LOG_DEFAULT, "Cannot find '%.*s'. Looked in the following locations:", static_cast<int>(SharedConstants::MONO_SGEN_SO.length ()), SharedConstants::MONO_SGEN_SO.data ());

	for (const char *od : AndroidSystem::override_dirs) {
		if (od == nullptr)
			continue;
		log_fatal (LOG_DEFAULT, "  %s", optional_string (od));
	}

	for (const char *app_lib_dir : AndroidSystem::app_lib_directories) {
		log_fatal (LOG_DEFAULT, "  %s", optional_string (app_lib_dir));
	}
