...
namespace Xamarin.Android.Tasks.New
{
	...
	class TypeMappingDebugNativeAssemblyGenerator : LlvmIrComposer
	{
		...
		protected override void Construct (LlvmIrModule module)
		{
			...
			module.AddGlobalVariable (TypeMapSymbol, type_map, LLVMIR.LlvmIrVariableOptions.GlobalConstant);

			if (managedToJavaMap.Count > 0) {
				module.AddGlobalVariable (ManagedToJavaSymbol, managedToJavaMap, LLVMIR.LlvmIrVariableOptions.LocalConstant);
			}

			if (javaToManagedMap.Count > 0) {
				module.AddGlobalVariable (JavaToManagedSymbol, javaToManagedMap, LLVMIR.LlvmIrVariableOptions.LocalConstant);
			}
		}
