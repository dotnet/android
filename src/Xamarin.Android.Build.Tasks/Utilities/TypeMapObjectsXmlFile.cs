...
class TypeMapObjectsXmlFile
{
	...
	void WriteTypeMapDebugEntry (XmlWriter xml, TypeMapDebugEntry entry)
	{
		...
		//if (entry.DuplicateForJavaToManaged != null)
		//	xml.WriteAttributeStringIfNotDefault ("duplicate-for-java-to-managed", entry.DuplicateForJavaToManaged.Key);
	...
	static void ImportDebugData (XElement root, TypeMapObjectsXmlFile file)
	{
		...
		if (javaToManaged is not null) {
			...
			//HandleDuplicates (file.JavaToManagedDebugEntries);
		}
		...
		if (managedToJava is not null) {
			...
			//HandleDuplicates (file.ManagedToJavaDebugEntries);
		}
	}

	static void HandleDuplicates (List<TypeMapDebugEntry> entries)
	{
		foreach (var entry in entries.Where (e => e.DuplicateForJavaToManagedKey.HasValue ())) {
			// Find the entry with the same key
			var dupe = entries.FirstOrDefault (e => e.Key == entry.DuplicateForJavaToManagedKey);

			if (dupe is null)
				throw new InvalidOperationException ($"Duplicate entry '{entry.DuplicateForJavaToManagedKey}' not found");

			entry.DuplicateForJavaToManaged = dupe;
		}
	}
	...
	static TypeMapDebugEntry FromDebugEntryXml (XElement entry, bool isMonoAndroid)
	{
		...
		//var duplicateForJavaToManagedKey = entry.GetAttributeOrDefault ("duplicate-for-java-to-managed", string.Empty);
		...
		return new TypeMapDebugEntry {
			...
			//DuplicateForJavaToManagedKey = duplicateForJavaToManagedKey,
			IsInvoker = isInvoker,
