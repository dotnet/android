...
interface IZipArchive : IDisposable
{
	...
	void MoveEntry (TaskLoggingHelper log, string oldEntry, string newEntry);
}
...
class ZipArchiveDotNet : IZipArchive
{
	...
	ZipArchiveDotNet (string archive, FileMode filemode)
	{
		// TODO: Translate FileMode to ZipArchiveMode
		Archive = ZipFile.Open (archive, ZipArchiveMode.Update);
	}
	...
	public static IZipArchive Create (TaskLoggingHelper log, string archive, FileMode filemode)
	{
		if (crc_field is null) {
			log.LogDebugMessage ($"ZipArchiveDotNet: Could not find private CRC field, falling back to libZipSharp.");
			throw new NotSupportedException ("crc_field is not supported on this platform.");
			//return new ZipArchiveEx (archive, filemode);
		}
		...
		if (comp_field is null) {
			...
			throw new NotSupportedException ("comp_field is not supported on this platform.");
			//return new ZipArchiveEx (archive, filemode);
		}

		return new ZipArchiveDotNet (archive, filemode);
	}
	...
	{
		if (!FileNeedsUpdating (filename, archiveFileName, compression)) {
	...
	public void FixupWindowsPathSeparators (TaskLoggingHelper log)
	{
		...
		foreach (var entry in malformed_entries) {
			...
			if (name != entry.FullName) {
				...
				MoveEntry (log, entry.FullName, name);
			}
		}
	}
	...
	public void MoveEntry (TaskLoggingHelper log, string oldPath, string newPath)
	{
	...
	bool FileNeedsUpdating (string filename, string archiveFileName, CompressionLevel compression)
	{
		...
		if (GetCompressionLevel (entry) != compression)
			return true;

		var last_write = File.GetLastWriteTimeUtc (filename);

		return WithoutMilliseconds (entry.LastWriteTime.DateTime) < WithoutMilliseconds (last_write);
	}
	...
	static CompressionLevel ConvertCompressionMethod (Xamarin.Tools.Zip.CompressionMethod method)
	{
		return method switch {
			Xamarin.Tools.Zip.CompressionMethod.Store => CompressionLevel.NoCompression,
			Xamarin.Tools.Zip.CompressionMethod.Deflate => CompressionLevel.Optimal,
