...
partial class AssemblyStoreGeneratorNew
{
	...
	string Generate (string baseOutputDirectory, AndroidTargetArch arch, List<AssemblyStoreAssemblyInfoNew> infos)
	{
		...
		foreach (AssemblyStoreAssemblyInfoNew info in infos) {
			...
			ulong name_with_ext_hash = LLVMIR.LlvmIrComposer.GetXxHash (Path.GetFileName (info.SourceFile.Name), is64Bit);
			ulong name_no_ext_hash = LLVMIR.LlvmIrComposer.GetXxHash (Path.GetFileNameWithoutExtension (info.SourceFile.Name), is64Bit);
			index.Add (new AssemblyStoreIndexEntry (name_with_ext_hash, desc.mapping_index));
			index.Add (new AssemblyStoreIndexEntry (name_no_ext_hash, desc.mapping_index));
	...
	static (AssemblyStoreEntryDescriptor desc, ulong newPos) MakeDescriptor (AssemblyStoreAssemblyInfoNew info, ulong curPos)
	{
