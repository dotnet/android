...
namespace Xamarin.Android.Build.Tests {
	...
	public class TrimmableTypeMapBuildTests : BaseTest {
		...
		void AssertPostTrimR8InputsExcludeDeadFrameworkImplementor (string apkPath, string javaSourceDirectory, string acwMapPath, string proguardPrimaryPath)
		{
			const string deadManagedType = "Android.Speech.IRecognitionListenerImplementor";
			const string deadJavaName = "mono/android/speech/RecognitionListenerImplementor";
			...
			Assert.IsFalse (
				...
				$"{apkPath} should not contain the removed framework listener implementor in classes.dex.");
		}

		ISet<string> ReadDexClassNames (string apkPath)
		{
			...
			foreach (var entry in zip.Entries.Where (e => Regex.IsMatch (e.FullName, @"^classes(\d*)\.dex$", RegexOptions.CultureInvariant))) {
				...
				foreach (var javaClass in DexProfileReader.Read (memory.ToArray ()).Classes) {
					classNames.Add (javaClass.Name);
				}
			}
			return classNames;
		}
