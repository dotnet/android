...
public class GenerateNativeMarshalMethodSources : AndroidTask
{
	...
	public override bool RunTask ()
	{
		...
		// Always unregister the NativeCodeGenStateCollection to clean up, regardless
		// of whether we need the data.  When marshal methods are enabled, the inner
		// build has already generated the .ll files; when disabled, we generate
		// empty/minimal .ll files.  PInvoke preservation needs the state regardless.
	...
	void Generate (NativeCodeGenStateCollection? nativeCodeGenStates, string abi)
	{
		...
		// When marshal methods are enabled, the inner build has already generated the
		// marshal_methods.<abi>.ll file — skip generation here.
		if (EnableMarshalMethods) {
			Log.LogDebugMessage ($"Marshal methods .ll file for '{targetAbi}' was generated by the inner build, skipping outer build generation.");
			return;
		}

		// Marshal methods are disabled — generate empty/minimal .ll files
		var (assemblyCount, uniqueAssemblyNames) = GetAssemblyCountAndUniqueNames ();

		// Create the appropriate runtime-specific generator (disabled path — empty/minimal code)
