	...
	public class GetExtraPackages : Task
	{
		...
		public override bool Execute ()
		{
			...
			if (Directory.Exists (libProjects)) {
				foreach (var assDir in Directory.GetDirectories (libProjects)) {
					foreach (var importBaseDir in new string [] { LibraryProjectImportsDirectoryName, "library_project_imports", }) {
						string importsDir = Path.Combine (assDir, importBaseDir);
						string libpkg = GetPackageNameForLibrary (importsDir, Path.GetDirectoryName (assDir));
						if (libpkg != null)
