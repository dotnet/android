...
public class PreTrimmingFixLegacyBindings : AndroidTask
{
	...
	public bool FixResourceDesigner { get; set; }
	...
	public override bool RunTask ()
	{
		...
		FixLegacyResourceDesignerStep? designerStep = null;
		if (FixResourceDesigner) {
			designerStep = new FixLegacyResourceDesignerStep ();
			designerStep.Initialize (context);
		}
		var modified = new List<ITaskItem> ();

		// Resolve the app's selected bindings before references from libraries compiled
		// against older versions can load a different binding from a search directory.
		foreach (var item in Assemblies) {
		...
		foreach (var item in Assemblies) {
			...
			bool modifiedAssembly = step.FixAbstractMethods (assembly);
			if (designerStep != null) {
				modifiedAssembly |= designerStep.ProcessAssemblyDesigner (assembly);
			}
			if (modifiedAssembly) {
