...
public class BuildArchive : AndroidTask
{
	...
	public override bool RunTask ()
	{
		...
		using var apk = ZipArchiveDotNet.Create (Log, ApkOutputPath, FileMode.Open);
		...
		if (ApkInputPath is not null && File.Exists (ApkInputPath) && refresh) {
			...
			using (var packaged = new ZipArchiveEx (ApkInputPath, FileMode.Open)) {
				foreach (var entry in packaged.Archive) {
					...
					apk.AddEntry (ms, entryName, ConvertCompressionMethod (entry.CompressionMethod));
				}
			}
		}
	...
	void FixupArchive (IZipArchive zip)
	{
		...
		zip.MoveEntry (Log, "AndroidManifest.xml", "manifest/AndroidManifest.xml");
	}
	...
	System.IO.Compression.CompressionLevel GetCompressionLevel (ITaskItem item)
	{
		...
		if (compression.HasValue ()) {
			if (Enum.TryParse (compression, out CompressionMethod result))
				return ConvertCompressionMethod (result);
		}

		return ConvertCompressionMethod (UncompressedFileExtensionsSet.Contains (Path.GetExtension (item.ItemSpec)) ? uncompressedMethod : CompressionMethod.Default);
	}

	System.IO.Compression.CompressionLevel ConvertCompressionMethod (CompressionMethod method)
	{
		switch (method) {
