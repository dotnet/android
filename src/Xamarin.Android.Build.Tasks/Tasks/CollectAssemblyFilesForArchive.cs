...
public class CollectAssemblyFilesForArchive : AndroidTask
{
	...
	void AddAssemblies (DSOWrapperGenerator.Config dsoWrapperConfig, List<ITaskItem> apk, bool debug, bool compress, IDictionary<AndroidTargetArch, Dictionary<string, CompressedAssemblyInfo>>? compressedAssembliesInfo, string? assemblyStoreApkName)
	{
		...
		foreach (var kvp in assemblyStorePaths) {
			...
			AddFileToArchiveIfNewer (apk, wrappedSourcePath, inArchivePath);
		}
	}

	void DoAddAssembliesFromArchCollection (TaskLoggingHelper log, AndroidTargetArch arch, ITaskItem assembly, DSOWrapperGenerator.Config dsoWrapperConfig, List<ITaskItem> apk, bool debug, bool compress, IDictionary<AndroidTargetArch, Dictionary<string, CompressedAssemblyInfo>>? compressedAssembliesInfo, string compressedOutputDir, AssemblyStoreBuilder? storeBuilder)
	{
		...
		AddFileToArchiveIfNewer (apk, wrappedSourcePath, assemblyPath);
	...
	void AddFileToArchiveIfNewer (List<ITaskItem> apk, string wrappedSourcePath, string inArchivePath, object? value = null)
	{
		var item = new TaskItem (wrappedSourcePath);
		item.SetMetadata ("ArchivePath", inArchivePath);
	...
	void AddAssemblyConfigEntry (DSOWrapperGenerator.Config dsoWrapperConfig, List<ITaskItem> apk, AndroidTargetArch arch, string assemblyPath, string configFile)
	{
		...
		AddFileToArchiveIfNewer (apk, wrappedConfigFile, inArchivePath);
	}
