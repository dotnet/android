...
class TrimmableTypeMap
{
	...
	/// <summary>
	/// Activates a managed peer from a Java constructor call. Called by UCO constructor
	/// wrappers to match the legacy <c>ManagedPeer.Construct</c> behavior: for parameterless
	/// constructors, invokes the user's managed ctor (preserving side effects); for
	/// parameterized constructors, falls back to the activation ctor from the hierarchy.
	/// </summary>
	internal static void ActivatePeerFromJavaConstructor (
		[DynamicallyAccessedMembers (DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.NonPublicConstructors)]
		Type targetType,
		IntPtr jniSelf,
		string jniCtorSignature)
	{
		var reference = new JniObjectReference (jniSelf);

		var existingPeer = JniEnvironment.Runtime.ValueManager.PeekPeer (reference);
		if (existingPeer is not null) {
			var state = existingPeer.JniManagedPeerState;
		...
		// For parameterless constructors, prefer the user's parameterless ctor
		// to preserve side effects (e.g., field initialization).
		if (jniCtorSignature == "()V") {
			var paramlessCtor = targetType.GetConstructor (
				BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance,
				null,
				Type.EmptyTypes,
				null);
			if (paramlessCtor is not null) {
				JniEnvironment.Runtime.ValueManager.ActivatePeer (existingPeer, reference, paramlessCtor, null);
				return;
			}
		}

		// For parameterized constructors, fall back to activation ctor from the hierarchy.
		// Create the target type (not the base type) and invoke the base activation ctor.
		var activationCtor = FindActivationConstructor (targetType);
		if (activationCtor is not null) {
			var self = (IJavaPeerable) System.Runtime.CompilerServices.RuntimeHelpers.GetUninitializedObject (targetType);
			self.SetPeerReference (reference);
			activationCtor.Invoke (self, [jniSelf, JniHandleOwnership.DoNotTransfer]);
			return;
		}

		throw new NotSupportedException (
			$"Unable to find constructor matching JNI signature '{jniCtorSignature}' or activation constructor for type '{targetType.FullName}'.");
	}

	static readonly Type[] ActivationCtorTypes = [typeof (IntPtr), typeof (JniHandleOwnership)];

	[UnconditionalSuppressMessage ("Trimming", "IL2072", Justification = "Java.Lang.Object subclass constructors are preserved by the MarkJavaObjects trimmer step.")]
	[UnconditionalSuppressMessage ("Trimming", "IL2075", Justification = "Java.Lang.Object subclass constructors are preserved by the MarkJavaObjects trimmer step.")]
	static ConstructorInfo? FindActivationConstructor (
		[DynamicallyAccessedMembers (DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.NonPublicConstructors)]
		Type type)
	{
