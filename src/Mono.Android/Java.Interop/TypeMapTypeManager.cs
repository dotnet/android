...
namespace Java.Interop
{
	public static class TypeManager
	{
		...
		{
			IntPtr ptr = RuntimeNativeMethods.monodroid_TypeManager_get_java_class_name (class_ptr); // goes through levels to call Class_getName on objecct
		...
		internal static string? GetJniTypeName (Type type)
		{
			if (!ProxyTypeMap.TryGetValue (type, out Type? proxyType))
				return null;
			if (proxyType.GetCustomAttribute<TypeMapProxyBaseAttribute> () is not { } attr)
				return null;
			return attr.JniName;
		}
		...
		{
			ExternalTypeMap.TryGetValue (java_type_name, out Type? value);
			Console.WriteLine ($"Getting typemap for '{java_type_name}': '{value?.FullName ?? "null"}'");
			return value;
		}
		...
		internal static IJavaPeerable? CreateInstance (IntPtr handle, JniHandleOwnership transfer, Type? targetType)
		{
			...
			Debug.Assert (class_name ==  JNIEnv.GetClassNameFromInstance (handle));
			lock (TypeManagerMapDictionaries.AccessLock) {
				string currentClassName = class_name;
				while (class_ptr != IntPtr.Zero) {
					type = GetJavaToManagedTypeCore (currentClassName);
					if (type != null) {
						break;
					}
					...
					if (class_ptr != IntPtr.Zero) {
						currentClassName = GetClassName (class_ptr);
					}
				}
			}
