    ...
    public class AndroidClientHandler : HttpClientHandler
    {
        ...
        Task HandlePreAuthentication (HttpURLConnection httpConnection)
        {
            return Task.Factory.StartNew (() => {
                ...
                if (creds == null) {
                    if (true)
                        System.Console.WriteLine ($"Authentication using scheme {data.Scheme} requested but no credentials found. No authentication will be performed");
                    return;
                }
                ...
                if (auth == null) {
                    if (true)
                        System.Console.WriteLine ($"Authentication module for scheme '{data.Scheme}' not found. No authentication will be performed");
                    return;
                }
                ...
                if (authorization == null) {
                    if (true)
                        System.Console.WriteLine ($"Authorization module {auth.GetType ()} for scheme {data.Scheme} returned no authorization");
                    return;
                }

                if (true)
                    System.Console.WriteLine ($"Authentication header '{data.UseProxyAuthentication ? "Proxy-Authorization" : "Authorization"}' will be set to '{authorization.Message}'");
        ...
        async Task SetupRequestBody (HttpURLConnection httpConnection, HttpRequestMessage request)
        {
            ...
            httpConnection.SetRequestProperty ("Content-Length", $"{bytes.Count ()}");
            ...
            await Task.Factory.StartNew (() => {
                httpConnection.OutputStream.Write (bytes, 0, bytes.Count ());
