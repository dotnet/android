...
namespace Android.Runtime {
	...
	class AndroidRuntimeOptions : JniRuntime.CreationOptions {
				...
				bool jniAddNativeMethodRegistrationAttributePresent)
		{
			...
			ObjectReferenceManager  = RuntimeFeature.IsMonoRuntime
		...
		internal void AddPeer (IJavaPeerable value, IntPtr handle, JniHandleOwnership transfer, out IntPtr handleField)
		{
			...
			if (Logger.LogGlobalRef) {
				...
				if (RuntimeFeature.IsMonoRuntime) {
					RuntimeNativeMethods._monodroid_gref_log (message + "\n");
				} else {
					JniEnvironment.Runtime.ObjectReferenceManager.WriteGlobalReferenceLine (message);
				}
			}
		}
