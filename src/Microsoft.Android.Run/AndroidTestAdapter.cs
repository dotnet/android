    ...
    bool verbose) : ITestFramework, IDataProducer
{
	...
	Task<InstrumentationBundleResult> RunInstrumentationOnDeviceAsync (CancellationToken cancellationToken)
	{
		return Task.Run (() => {
	...
	Task<string?> PullTrxFileAsync (string devicePath, CancellationToken cancellationToken)
	{
		return Task.Run (() => {
			...
			var (exitCode, output, error) = AdbHelper.Run (adbPath, adbTarget, $"pull \"{devicePath}\" \"{localPath}\"", verbose);
