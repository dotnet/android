...
## Building .NET for Android
 ...
 6. Build the project:
        ...
        dotnet-local.cmd build Microsoft.Android.slnx
...
Opening `Microsoft.Android.slnx` in Visual Studio currently tends to hold file
...
`Microsoft.Android.Build.Tasks.slnx` instead.  But if you are working on changes
...
.NET for Android workload install with:
    ...
    dotnet-local.cmd build Microsoft.Android.slnx -t:BuildDotNet
...
Once `dotnet msbuild Microsoft.Android.slnx -t:Prepare` is complete,
.NET for Android workload packs can be built with:

    dotnet-local.cmd build Microsoft.Android.slnx -t:BuildDotNet,PackDotNet

Several `.nupkg` files will be output in `.\bin\Build$(Configuration)\nuget-unsigned`.

## Building Unit Tests

Once `dotnet-local.cmd build Microsoft.Android.slnx` has completed, the unit tests may
be built with e.g.:

    dotnet-local.cmd build Microsoft.Android-Tests.slnx /restore /p:Configuration=Debug /bl:bin\TestDebug\msbuild-build-tests.binlog
...
All NUnit-based unit tests can be executed via
    ...
    dotnet-local.cmd build Microsoft.Android.slnx /t:RunNUnitTests

### Listing NUnit Tests

In order to get a list of the tests you can use the `ListNUnitTests` target

    dotnet-local.cmd build Microsoft.Android.slnx /t:ListNUnitTests
...
You can run then a single (or a group) of tests using the `$(TEST)` MSBuild property.
    ...
    dotnet-local.cmd build Microsoft.Android.slnx /t:RunNUnitTests /p:TEST=Xamarin.Android.Build.Tests.Aapt2Tests.Aapt2Compile
