...
Errors reported by `aapt` are outside of .NET for Android's control, so a general error code of APT0000 is used to report the exact message coming from `aapt`.
