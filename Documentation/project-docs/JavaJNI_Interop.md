...
Java Callable Wrappers are generated only for types that derive from
...
then used by a variety of tasks, Java Callable Wrappers being only one
of them. 
...
class while Xamarin.Android is built, from which our example
...
the `Xamarin.Android` runtime twice during application startup - once
...
that any assembly (either from Xamarin.Android or a third party nuget)
