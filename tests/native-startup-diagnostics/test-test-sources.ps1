...
if (-not $IsWindows) { throw 'This pipeline configuration regression requires Windows.' }
...
try {
    ...
    $lines = @(& "$scripts/guest-readiness-test-sources.ps1")
    ...
    foreach ($case in @('prior-args', 'malformed-optin', 'relative-config', 'missing-config', 'different-config')) {
        ...
        try { & "$scripts/guest-readiness-test-sources.ps1" | ForEach-Object { $emitted.Add($_) } }
