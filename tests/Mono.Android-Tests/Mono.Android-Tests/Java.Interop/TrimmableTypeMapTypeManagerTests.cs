...
namespace Java.InteropTests
{
	...
	public class TrimmableTypeMapTypeManagerTests
	{
		...
		public void JavaProxyObject_ObjectMethodsUseJavaIdentitySemantics ()
		{
			...
			using var values = new JavaObjectArray<object> (2);
			values [0] = value;
			values [1] = other;

			var localProxy = JniEnvironment.Arrays.GetObjectArrayElement (values.PeerReference, 0);
			var localOtherProxy = JniEnvironment.Arrays.GetObjectArrayElement (values.PeerReference, 1);
			try {
				...
				try {
					...
					try {
						...
						Assert.AreEqual ("net/dot/jni/internal/JavaProxyObject", JNIEnv.GetClassNameFromInstance (localProxy.Handle));
