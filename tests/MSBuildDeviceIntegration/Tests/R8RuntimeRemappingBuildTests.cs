	...
	public class R8RuntimeRemappingBuildTests : BaseTest
	{
		...
		public void MultiRidUsesOneR8Mapping (AndroidRuntime runtime, bool explicitPrimaryRid)
		{
			...
			var proj = new XamarinAndroidApplicationProject {
				IsRelease = true,
				OtherBuildItems = {
					new AndroidItem._AndroidRemapMembers ("explicit-remap.xml") {
						TextContent = () => """
							<replacements>
							  <reverse-type from="example/ExplicitResidual" to="example/ExplicitOriginal" />
							</replacements>
							""",
					},
				},
			...
			if (runtime == AndroidRuntime.NativeAOT) {
				...
				Assert.AreEqual (2, remapFiles.Length);
				foreach (string remapFile in remapFiles) {
					StringAssert.Contains (
						"""<reverse-type from="example/ExplicitResidual" to="example/ExplicitOriginal" />""",
						File.ReadAllText (remapFile),
						$"The outer explicit remap input must reach the per-RID NativeAOT table: {remapFile}");
				}
