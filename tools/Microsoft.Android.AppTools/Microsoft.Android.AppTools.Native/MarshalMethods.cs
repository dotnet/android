...
sealed class MarshalMethodsManagedClass_V2
{
	...
	public MarshalMethodsManagedClass_V2 (ILogger log, BinaryReader reader, AnELF elf, ISymbolEntry symbolEntry)
	{
		...
		sizeSoFar += NativeHelpers.ReadField (reader, ref token, sizeSoFar, is64Bit);
		sizeSoFar += NativeHelpers.ReadField (reader, ref klass, sizeSoFar, is64Bit);
	}
}
...
sealed class MarshalMethodName_V2
{
	...
	public MarshalMethodName_V2 (ILogger log, BinaryReader reader, AnELF elf, ISymbolEntry symbolEntry)
	{
		...
		//   - pointer to name
		sizeSoFar += NativeHelpers.ReadField (reader, ref id, sizeSoFar, is64Bit);
...
abstract class MarshalMethods
{
	...
	{
		Utils.Log?.DebugLine ($"{LogTag} checking whether {elf.FilePath} contains supported marshal methods structures");
		...
		{
			Utils.Log?.DebugLine ($"Marshal methods {Utils.AreOrNot (retval)} supported in {elf.FilePath}");
			return retval;
		}
	}
	...
	{
		Utils.Log?.DebugLine ($"{LogTag} checking for required {formatName} symbols in {elf.FilePath}");
		bool allGood = true;
		foreach (string symbol in symbolNames) {
			Utils.Log?.Debug ($"  '{symbol}'");
			if (!elf.HasSymbol (symbol)) {
				Utils.Log?.DebugLine (" [missing]");
				allGood = false;
			} else {
				Utils.Log?.DebugLine (" [found]");
			}
		}
	...
	{
		Utils.Log?.WarningLine ($"{LogTag} reader does not support libxamarin-app.so format version 0x{format_tag:x}");
	}
}
...
class MarshalMethods_V2 : MarshalMethods
{
	...
	void ReadClassNames (AnELF elf, List<string> names)
	{
		...
		while (stream.Position < stream.Length) {
			...
			ulong pointerOffset = NativeHelpers.GetPadding<string> (sizeSoFar, is64Bit) + entryOffset;
