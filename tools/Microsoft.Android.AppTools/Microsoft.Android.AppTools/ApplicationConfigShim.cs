...
sealed class ApplicationConfigShim
{
	...
	public ApplicationConfigShim (ApplicationConfig_V2 appConfig)
	{
		...
		UsesMonoLlvm                                   = Utils.YesNo (appConfig.uses_mono_llvm);
		UsesMonoAot                                    = Utils.YesNo (appConfig.uses_mono_aot);
		AotLazyLoad                                    = Utils.YesNo (appConfig.aot_lazy_load);
		UsesAssemblyPreload                            = Utils.YesNo (appConfig.uses_assembly_preload);
		BrokenExceptionTransitions                     = Utils.YesNo (appConfig.broken_exception_transitions);
		InstantRunEnabled                              = Utils.YesNo (appConfig.instant_run_enabled );
		JniAddNativeMethodRegistrationAttributePresent = Utils.YesNo (appConfig.jni_add_native_method_registration_attribute_present);
		HaveRuntimeConfigBlob                          = Utils.YesNo (appConfig.have_runtime_config_blob);
		HaveAssembliesBlob                             = Utils.YesNo (appConfig.have_assemblies_blob);
		MarshalMethodsEnabled                          = Utils.YesNo (appConfig.marshal_methods_enabled);
