...
| **Double-checked locking requires `volatile`** | The double-checked locking (DCL) pattern in .NET requires the shared field to be `volatile` (or use `Volatile.Read`/`Volatile.Write`). Without `volatile`, the .NET memory model on ARM64 allows the write to the field to become visible to other threads *before* the object's constructor or initialization code completes. A thread can see a non-null reference to a half-initialized object. This is a **correctness bug**, not just a best practice. If a PR introduces or modifies a DCL pattern, verify: (1) the field is `volatile`, (2) all initialization (including post-construction setup like `RegisterNatives()`) completes *before* the field is assigned, (3) there is no window where another thread can observe the singleton before initialization is fully done. |
